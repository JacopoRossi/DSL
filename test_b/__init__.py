...
# V2 blueprint (formerly DSL) - Service-Oriented Aerospace DSL
v2_bp = Blueprint(
    'v2', __name__,
    url_prefix='/v2',
...
@v2_bp.route('/api/save_step', methods=['POST'])
def save_step():
...
@v2_bp.route('/api/get_config', methods=['GET'])
def get_config():
    """Get current configuration"""
    return jsonify(dsl_config)


@v2_bp.route('/api/download', methods=['GET'])
def download():
...
@v2_bp.route('/api/reset', methods=['POST'])
def reset():
